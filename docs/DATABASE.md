...
6. **`products`** : Cette table stocke les informations sur les différents produits. Chaque produit a un `id` unique (PK), un `product_name`, un `product_description`, un `product_price` et un `product_category_id`. Le `product_category_id` est une clé étrangère qui fait référence à l'`id` de la table `product_category`. Cela signifie qu'un produit est associé à une catégorie de produit.
7. **`product_pictures`** : Cette table stocke les informations sur les différentes images de produits. Chaque image a un `id` unique (PK), un `product_path_img` et un `product_id`. Le `product_id` est une clé étrangère (FK) qui fait référence à l'`id` de la table `products`. Cela signifie qu'une image est associée à un produit.
...
9. **`doctor_pictures`** : Cette table stocke les informations sur les différentes images de médecins. Chaque image a un `id` unique (PK), un `doctor_path_img` et un `doctor_id`. Le `doctor_id` est une clé étrangère (FK) qui fait référence à l'`id` de la table `doctors`. Cela signifie qu'une image est associée à un médecin.
...
CREATE TABLE IF NOT EXISTS `sections` (
    `id` INT AUTO_INCREMENT NOT NULL,
    PRIMARY KEY (`id`),
    `section_name` VARCHAR(110) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `product_pictures` (
    `id` INT AUTO_INCREMENT NOT NULL,
    PRIMARY KEY (`id`),
    `product_path_img` VARCHAR(255) NOT NULL,
    `product_id` INT,
...
CREATE TABLE IF NOT EXISTS `doctor_pictures` (
    `id` INT AUTO_INCREMENT NOT NULL,
    PRIMARY KEY (`id`),
    `doctor_path_img` VARCHAR(255) NOT NULL,
    `doctor_id` INT,
...
INSERT INTO 
`sections` (`section_name`, `section_description`, `section_category_id`) 
VALUES
...
INSERT INTO
`products` (`product_name`, `product_description`, `product_price`, `product_category_id`)
VALUES 
...
INSERT INTO `product_pictures` (`product_path_img`, `product_id`) VALUES
('drink_me.webp', 1),
('eat_me.webp', 2),
('upelkuchen.webp', 3),
...
INSERT INTO `doctor_pictures` (`doctor_path_img`, `doctor_id`) VALUES
('alice-doctor.webp', 1),
